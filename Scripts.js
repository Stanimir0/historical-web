(() => {
  ...
  const initLinks = () => {
    ...
    // Default: if we fail to find assets, send users to Releases.
  ...
  const setDownloadButtonsFromRelease = (release) => {
    ...
    if (els.downloadText) {
      ...
      if (windowsOk && androidOk) {
        els.downloadText.textContent = t(currentLang, "release.text.both") || els.downloadText.textContent;
      } else if (windowsOk || androidOk) {
        els.downloadText.textContent = t(currentLang, "release.text.one") || els.downloadText.textContent;
      } else {
        els.downloadText.textContent = t(currentLang, "release.text.none") || els.downloadText.textContent;
      }
    }
  ...
  const loadLatestRelease = async () => {
    try {
      ...
      // Ignore; we already have good fallbacks.
    }
  };
